...
As shown in the above plots, as level increase, the length of the curve become longer and the curve folds
more tensely. If a Hilbert curve with level 11 is used to map to human chromosome 1, the resolution would be
`249250621/2^11/2^11` (approximately 60bp).
...
curve. It provides functions to add low-level graphics and also supports both "low-resolution"
...
Reproduce the ip... The data is from http://www.iwik.org/ipcountry/
...
hc = HilbertCurve(min(chinese_dynasty[[2]]), max(chinese_dynasty[[3]]), title = "中国朝代（1122 B.C. ~ 1911 A.D.）", 
...
```{r, eval = on_cluster, echo = 1:3, fig.keep = "none"}
...
```{r, eval = on_cluster, plot.keep = "none", echo = 1}
...
```{r, eval = on_cluster, fig.keep = "none", echo = 1:6}
...
```{r, eval = on_cluster, fig.keep = "none", echo = 1:5}
...
```{r, eval = on_cluster, fig.keep = "none", echo = 1:5}
df = read.table("~/HilbertCurveTest/cnv.txt", sep = "\t")
df = df[c(1, 2, 2, 3)]
...
The map can be directly added as a layer, but it would mixed the plot

```{r, eval = on_cluster}
# you can use `hc` generated before, the first following line
# is only used to run wheng generating the vignette
hc_map(hc)
```
