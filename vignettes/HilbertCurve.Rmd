...
correspond to the layer which is going to be added):
...
may be difficult to use but it is powerful to represent different features. In following example, we 
...
hc_layer(hc, x1 = 750, x2 = 850, col = "#0000FF40",
    overlay = function(r0, g0, b0, r, g, b, alpha) {
        l = r0 > 0 # if there is peak and covered by gene
        tmp = r0[l]
        r0[l] = g0[l]
...
The self-defined `overlay` should accept sevel argument for which, `r0`, `g0` and `b0` correspond to the 
layer which is already added, `r`, `g`, `b` and `alpha` correspond to the layer which are going to be added.
The elements of e.g. `r` only correspond to the pixels that inside the regions in the new layer.

**circlize** provides a function `col2value()` which transforms back from colors to the original values. 
It is usefull to integrate with `overlap` to change the color theme.

In following example, color theme is from white to red.
...
Next we first select the pixels which represent the regions that are already added. Since the background
is white (red, green and blue channel all have value of 1), we select the non-white pixels. Then we create
a new color theme which change from white to blue and update the corresponding colors.
...
is overlapped with gene body, the color theme is changed to orange.
