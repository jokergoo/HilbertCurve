...
One major feature is multiple chromosomes (or genomic categories) can be put onto one same curve.
...
genomic features, they can define a color overlay function to enhance the overlapping parts. In following example, when a region for H3K36me3 mark
is overlapped with gene body, the color theme is changed to purple.
...
Visualizing methylation. Blue corresponds to un-methylation, red corresponds to full-methylation.
One thing which is very important here is that methylation is always measured at single CpG
sites, you need to set the mode for averaging to `absolute` to average only on CpG sites.
...
In following examples, copy number loss and gain for 22 human chromosomes are visualized. Data is from (Database of Genomic Variants, CNV map)[http://dgv.tcag.ca/dgv/app/downloads]. 
...
to put them together. Remember to set `newpage` to `FALSE`.
