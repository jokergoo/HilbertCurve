...
Also there are some data points in (1, 256) which are close to data points in (768, 1024)(bottom left or top right area in the heatmap),
but it is only because the curve folds back. This exception can be easily distinguished 
...
by `start_from` option:

```{r, fig.width = 12, fig.height = 3, echo = FALSE}
grid.newpage()
pushViewport(viewport(layout = grid.layout(nr = 1, nc = 4)))
pushViewport(viewport(layout.pos.row = 1, layout.pos.col = 1))
hc = HilbertCurve(1, 16, level = 2, reference = TRUE, newpage = FALSE, start_from = 'bottomleft', title = "start_from = 'bottomleft'")
upViewport()
pushViewport(viewport(layout.pos.row = 1, layout.pos.col = 2))
hc = HilbertCurve(1, 16, level = 2, reference = TRUE, newpage = FALSE, start_from = 'topleft', title = "start_from = 'topleft'")
upViewport()
pushViewport(viewport(layout.pos.row = 1, layout.pos.col = 3))
hc = HilbertCurve(1, 16, level = 2, reference = TRUE, newpage = FALSE, start_from = 'bottomright', title = "start_from = 'bottomright'")
upViewport()
pushViewport(viewport(layout.pos.row = 1, layout.pos.col = 4))
hc = HilbertCurve(1, 16, level = 2, reference = TRUE, newpage = FALSE, start_from = 'topright', title = "start_from = 'topright'")
upViewport()
upViewport()
```
...
Graphic parameters can be set by `gp`. Note under this mode, the size of points can only be controled by `np` argument.
...
by the circle in the end is not fully covered by user's intervals, thus, averaging is applied here. E.g. if only
...
Basically polygons are quite similar to the rectangles in previous section. The major difference is 1. borders
can be set for polygons; 2. there is only single colors in one polygon (remember for rectangles, the color can 
change for the ends of the regions if the corresponding segment is not completely covered by the input region).
...
```{r}
hc = HilbertCurve(1, 1000, level = 9, mode = "pixel")
x = sample(1000, 100)
...
Also borders can be drawn by setting `border` to `TRUE` or a vector of colors.

```{r}
hc = HilbertCurve(1, 100, level = 9, mode = "pixel")
hc_layer(hc, ir, border = TRUE)
```
...
may be difficult to use but it is powerful for representing different features. In following example, we 
...
hc_layer(hc, x1 = 750, x2 = 850, col = "#0000FF40",
    overlay = function(r0, g0, b0, r, g, b, alpha) {
        ...
        tmp = r0[l]
        r0[l] = g0[l]
        g0[l] = tmp  # red -> green, just switch red and green channel
...
**circlize** provides a function `col2value()` which transforms back from colors to the original values (it can be thought
as the reversed function of `colorRamp2()`). 
It is usefull to integrate with `overlay` to change the color theme.

In following example, default color theme is white-red.
...
Next we change the color theme in the overlapped areas to white-blue. To do this,
we first select the pixels that correspond to regions that have been already added.
Since background color is white, non-white pixels correspond to the regions that
have beed added. Then transform back to the original values by `col2value()` and finally change to 
another color theme.
...
If above example, actually you can realize such overlay by first split "red" regions into two parts
...
provides an easier way and more straightforward way to do it.
...
Under "pixel" mode, the curve will also be shown on the interactive device as raster image, thus, title
and legends can be added as well.
...
Reproduce [the visualization for IPv4 address](http://datadrivensecurity.info/blog/posts/2015/Jan/mapping-ipv4-address-in-hilbert-space/). The CIDR data is from http://www.iwik.org/ipcountry/.
...
```{r, eval = FALSE}
# Following code does not run when building the vignette due to different font family for
# Chinese in different OS.
...
<p><center><img src = "chinese_dynasty.png" /></center></p>
...
One major feature is multiple chromosomes (or more general genomic categories) can be put onto one same curve.
...
Next figure visualizes GC percent on chromosome 1, under "normal" mode. The GC content per 1KB window
...
Or visualize GC percent on chromosome 1, under "pixel" mode:
...
Background is not necessary to be the whole chromosomes, it can also be a self-defined regions.
...
Following visualizes different histone marks on human chromosome 1. Additionally, gene regions are added as a grey layer.
...
regions between layer for gain events and layer for loss events is changed to purple by defining `overlay` argument.
...
The map can be overlayed to the first plot so that you only need to look at one plot.
...
If colors of chromosomes sometimes defuse the original plot, you can only draw borders
of chromosomes.
...
hc_map(hc, add = TRUE, border = "black")
...
```{r, eval = grepl("tbi", Sys.info()["nodename"]) & Sys.info()["user"] == "guz", fig.keep = "none", echo = 2:34}
...
plot_curve = function(column, ...) {
    ...
    df = df[df[[column]] > 0, , drop = FALSE]
    hc_layer(hc, df, col = "green", grid_line = 3, grid_line_col = "grey",
        overlay = function(r0, g0, b0, r, g, b, alpha) {
            l = !(r0 == 1 & g0 == 1 & b0 == 1)
            r[l] = 160/255
            g[l] = 32/255
